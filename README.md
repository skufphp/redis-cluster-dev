...
Инфраструктурный проект для локальной разработки **Redis Cluster** в Docker-окружении с полной автоматизацией через **Makefile**.

Кластер состоит из **6 нод** (`redis-node1-dev` … `redis-node6-dev`) и может использоваться множеством приложений, подключающихся через общую внешнюю Docker-сеть.
...
- **Версия**: Redis 8.6-alpine
- **Архитектура**: 6 нод (3 мастер + 3 реплики)
- **Память**: 2GB на ноду с политикой `allkeys-lru`
- **Персистентность**: AOF + RDB снапшоты
- **Сеть**: Внешняя Docker-сеть `redis-cluster-dev`
